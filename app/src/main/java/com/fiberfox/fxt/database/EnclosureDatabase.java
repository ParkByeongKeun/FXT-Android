...
public class EnclosureDatabase extends BaseDaoEnclosure<EnclosureDataBean> {
     ...
     * @param context 上下文环境
     */
    public EnclosureDatabase(Context context) {
        super(context);
    }

    private static class SpliceDataDaoInner{
        @SuppressLint("StaticFieldLeak")
        private static final EnclosureDatabase SPLICE_DATA_DAO = new EnclosureDatabase(mContext);
    }
     ...
     * @return  返回该类的对象
     */
    public static EnclosureDatabase getInstance(Context context) {
        mContext = context;
    ...
    @Override
    public void insert(EnclosureDataBean bean) {
        if (bean == null){
            return;
        }
        ContentValues values = new ContentValues();
//        values.put("id", bean.getId());
        values.put("coordinate", bean.getCoordinate());
        values.put("imagePath", bean.getImagePath());
        values.put("note", bean.getNote());
        values.put("data_time", bean.getDataTime());
        values.put("user", bean.getUser());

        db.insert("tb_enclosure_data", null, values);

        // 获取最后插入的行ID
        int lastId = -1;
        Cursor cursor = db.rawQuery("select last_insert_rowid() from tb_enclosure_data",null);
...
//        db.insert("tb_fnms_data", null, fiberValues);
    ...
    @Override
    public void update(EnclosureDataBean ofiDataBean) {
        //        String sql_ofi_data = "update tb_ofi_data set memo = " + newImagePath +" where _id = " + id;
        ContentValues values = new ContentValues();
//        values.put("id", bean.getId());
        values.put("coordinate", ofiDataBean.getCoordinate());
        values.put("imagePath", ofiDataBean.getImagePath());
        values.put("note", ofiDataBean.getNote());
        values.put("data_time", ofiDataBean.getDataTime());
        values.put("user", ofiDataBean.getUser());

        db.update("tb_enclosure_data",values,"_id = ?", new String[] {ofiDataBean.getId()});
//        db.execSQL(sql_ofi_data);
    }

    @Override
    public void deleteById(int id) {
        String sql_ofi_data = "delete from tb_enclosure_data where _id = " + id;
        db.execSQL(sql_ofi_data);
...
//        db.execSQL(sql_splice_fiber_data);
    ...
    @Override
    public List<EnclosureDataBean> selectAllSpliceData() {
        List<EnclosureDataBean> OFIDataBeanList = new ArrayList<>();
//        List<FiberBean> fiberBeanList = new ArrayList<>();
//        List<String> arrDate = new ArrayList<>();

        Cursor cur = db.rawQuery("SELECT * FROM tb_enclosure_data order by _id desc", null);
...
//                    if(cur.getString(9) != null && cur.getString(10) != null) {
                        EnclosureDataBean spliceValue = new EnclosureDataBean();
                        spliceValue.setId(cur.getString(0));
                        spliceValue.setCoordinate(cur.getString(1));
                        spliceValue.setImagePath(cur.getString(2));
                        spliceValue.setNote(cur.getString(3));
                        spliceValue.setDataTime(cur.getString(4));
                        spliceValue.setUser(cur.getString(5));
...
//                    }
    ...
    @Override
    public EnclosureDataBean selectResultDataByFileName(String sn) {
        return null;
    }
